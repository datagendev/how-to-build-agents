...
- [Agent To Create: Personalized PDF One-Pager Factory](#agent-to-create-personalized-pdf-one-pager-factory)
...
| Prospect one-pager | `onepager-template.md`, `client-value-props.md`, `tiering-rules.md` |
...
## Agent To Create: Personalized PDF One-Pager Factory

This is a real agent we can build together. It takes what you already do manually -- researching prospects and building personalized content -- and turns it into a scalable pipeline. Instead of spending 20-30 minutes per prospect on a custom website, every lead in your outreach sequence gets a personalized PDF one-pager automatically.

### The Problem

In cold outreach, personalization wins. But it doesn't scale. Right now, building a custom piece for each prospect means:

1. Research the prospect and their company
2. Understand their pain points based on role, industry, and company stage
3. Connect those pains to the client's solution
4. Build a personalized deliverable
5. Attach it to the outreach sequence

At 20-30 minutes per prospect, you can only do this for top-tier leads. The other 90% get generic outreach.

### Why PDF > Website

- **Higher perceived effort** -- a PDF attachment feels like someone made it for them
- **Works offline** -- prospects save and share internally
...
**Name**: `prospect-onepager`

**Trigger**: Webhook -- lead enters HeyReach/Instantly campaign

**Input**: Lead data (name, company, title, LinkedIn URL, domain)

**Output**: Branded PDF one-pager uploaded to cloud storage, shareable URL returned for sequence variable insertion

### Tiering Logic (the agent judgment part)

Not every prospect deserves the same level of effort. The agent decides:

| Tier | Who | What they get |
|------|-----|--------------|
| **Tier 1** (decision makers) | VP+, C-suite, founders | Full personalized PDF: company-specific pain points, ROI estimate, relevant case study, custom messaging |
| **Tier 2** (influencers) | Directors, senior managers | Lighter PDF: role-specific content, industry proof points |
| **Tier 3** (volume) | Everyone else | Template PDF: company name + industry-specific stats swapped in |

### Step-by-Step Workflow

```
Step 1:  Receive lead data via webhook
         Parse payload: name, company, title, LinkedIn URL, domain
         Determine tier based on title + seniority rules
         -> tmp/lead_input.json

Step 2:  Research the prospect (batch: 5 at a time)
         Scrape company website via Firecrawl
         Pull LinkedIn profile data
         Identify: company stage, industry, recent news, tech stack
         -> tmp/research_{domain}.json

Step 3:  Identify pain points
         Using @context/pain-point-mapping.md:
         Cross-reference role + industry + company stage against known pain patterns
         For Tier 1: deep analysis with company-specific signals
         For Tier 2/3: pattern-match to industry-level pains
         -> tmp/pain_points_{domain}.json

Step 4:  Match to client value props
         Using @context/client-value-props.md:
         Map each pain point to a specific solution, case study, or proof point
         Select the strongest 2-3 connections
         -> tmp/value_match_{domain}.json

Step 5:  Generate PDF content
         Using @context/onepager-template.md for layout:
         - Prospect's company name and logo (scraped from website)
         - 2-3 personalized pain points with evidence
         - Matched solutions with proof points
         - Relevant case study snippet
         - Clear CTA
         Tier determines depth and detail level
         -> tmp/onepager_content_{domain}.json

Step 6:  Build and export PDF
         Use /pdf skill to generate branded, styled PDF
         Apply client branding (colors, logo, fonts)
         -> tmp/onepager_{domain}.pdf

Step 7:  Upload and return URL
         Upload PDF to cloud storage (Cloudflare R2 or S3)
         Generate shareable link
         Return URL for insertion into outreach sequence variable
         -> pdfs.client-domain.com/{domain}.pdf
```
...
| File | Purpose |
|------|---------|
| `onepager-template.md` | Layout and structure -- what sections go where, how long each should be |
| `client-value-props.md` | The client's solutions, case studies, proof points -- what you're selling |
| `pain-point-mapping.md` | Role + industry + stage -> likely pain points. This is YOUR outreach expertise encoded |
| `tiering-rules.md` | How to classify prospects into Tier 1/2/3 based on title and seniority |
| `brand-assets/` | Client logo, colors, fonts for PDF styling |

### Why This Can't Be Done in n8n/Clay

| Capability | n8n/Clay | This agent |
|-----------|----------|-----------|
| Scrape a website | Yes | Yes |
| Read a website and *understand what they struggle with* | No | Yes -- reasoning over content |
| Match pain points to value props | No | Yes -- judgment, not keyword matching |
| Generate a coherent, personalized narrative | No | Yes -- the PDF tells a story |
| Tier prospects by seniority and adjust depth | Partially (rules) | Yes -- semantic understanding of titles |
| Produce a designed PDF (not a template fill) | No | Yes -- creative artifact generation |
...
|-----------|---------------|
| **Job-to-be-done** | The agent does one job: turn a lead into a personalized PDF one-pager |
| **Develop by doing** | You'll manually research 3-5 prospects and build their one-pagers first, capturing exactly what you look for and how you write |
| **Context is the enemy** | Research results saved to `tmp/` per prospect, processed individually -- never all in context at once |
| **Plan before act** | Tiering decision (Step 1) determines the execution path for all downstream steps |
| **Validate with hooks** | Every PDF must have: prospect name, at least 2 pain points, at least 1 case study reference, a CTA |
| **Encoded expertise** | The pain-point mapping and tiering rules ARE the product -- they encode your outreach judgment |

### How to Build It (Applying the Workflow)

1. **Step 0**: Write `pain-point-mapping.md` together -- manually research 3-5 real prospects from a current campaign, capture what you look for and how you connect it to the client's solution
2. **Step 3**: Prototype the workflow -- `/clear`, then manually research a prospect, write the one-pager content, generate the PDF in Claude Code
3. **Step 4**: Codify the trace into the agent definition with indexed steps
4. **Step 5**: Test it on 5 prospects, iterate until the PDFs look right
5. **Steps 6-8**: Deploy to DataGen with a webhook trigger so it fires when leads enter campaigns

Once deployed, every lead that enters a HeyReach/Instantly campaign automatically gets a personalized PDF. You go from 5 custom pieces a week to 50 a day -- and the quality scales with the tiering logic, not just volume.
